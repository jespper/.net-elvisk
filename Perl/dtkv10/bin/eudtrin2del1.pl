...
sub speciale_create {
	...
	while ( $line = shift( @eta )) {  
		...
		if ( $line =~ /^<FAG   >(\d+)\s+(.+)$/) {
            ...
            print "\n.varighed..".$varighed;
            #MENTALT RETARDET Print af array
            print "\n.resultatformer..\n";
            foreach my $result (@resultatformer) {
                print "$result,\n";
            }
            print "\n.pinde..\n";
            my @pind;
        ...
        #nok ikke nødvendig når du ikke bruger den
		...
		if ( $line =~ /^<PIND  >(\d*)/ ) {
			...
			#printf("ETA: %s\n", $eta[0] );
            while ( defined $eta[0] && $eta[0] =~ /<PIND\+ >(.*)/ ) { 
			    #printf("ETA: %s\n", $eta[0] );
                my $tmpValue = $eta[0];
                push(@pinde, [$tmpValue]);
...
#Opretter elevtype i med specialer, fag og faginstanser
#i databasen.
...
sub elevtype_read {
	...
	while( my @tmparr = $sth->fetchrow_array() ) {
		$tmparr[0] =~ /^'(.*)'$/; #Strip of quotes '' from DB
		push(@{$elev_ref}, $1);
	}
	return($linjer);
}
...
#my $dbh;
#my $sth;
$sth = $dbh->prepare("SELECT ordning_id FROM ordning ORDER BY revision_dato DESC LIMIT 1");
...
my @elevtyper;
my @elev;
my $linjer  = $sth->rows; #Er der i DBI en smartere måde at indlæse et array?
while( @tmparr = $sth->fetchrow_array() ) {
